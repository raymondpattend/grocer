 ...
 * cost of each identification. Resolves to an empty outcome when the photo isn't
 * grocery-related, the model fails, or OpenAI isn't configured — so it never
 * fails (the `never` error channel).
 *
 ...
 */
export function identifyItemWithAI(
  image: string,
  mimeType: string,
  options: {
    distinctId?: string;
...
): Effect.Effect<IdentifyOutcome, never, OpenAi | AppConfig | Exec> {
  return Effect.gen(function* () {
    const { env } = yield* AppConfig;
    const exec = yield* Exec;
    const openai = yield* OpenAi;

    if (!image || !openai.apiKey) return EMPTY_OUTCOME;
    ...
    return yield* openai
      ...
      .pipe(
        Effect.map((completion): IdentifyOutcome => {
          ...
          if (outputText) {
            try {
              const decoded = JSON.parse(outputText) as {
                kind?: string;
                item?: { name?: string; category?: string; confidence?: number } | null;
                items?: Array<{
                  name?: string;
                  category?: string;
                  quantity?: string;
          ...
          captureAiGeneration({
            ...
            properties: {
              ...
              grocer_identify_kind:
                result.items.length > 0 ? "list" : result.item ? "item" : "none",
        ...
        Effect.catchAll((error) =>
          Effect.sync((): IdentifyOutcome => {
            ...
            console.warn("OpenAI identify-item skipped:", cause);
